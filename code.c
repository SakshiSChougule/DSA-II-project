...
CodeWordNode CodeWordNodeCreate(TreeNode trn){
    ...
    int byte_num = (bit_num + 8 - 1) / 8;  

    code* cwn = (code*) malloc(sizeof(struct _CodeWordNode));
    assert(cwn != NULL);
    ...
    // but all other(if exist) are exact 8 bits
    ...
    while (! IsRootNode(current)){
        ...
        if (buffer_num == 8){
            cwn->s[s_idx] = buffer & 255;   // remove any extra bits longer than 8 bits
            s_idx -= 1;
    ...
    // if depth is multiple of 8, then finish, s_idx = -1
